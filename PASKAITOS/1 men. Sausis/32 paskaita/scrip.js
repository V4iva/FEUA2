...
btn[1].onclick = () => {
    ...
    roundCircle.classList.add('circle')

    main.appendChild(roundCircle)
}
btn[2].onclick = () => {
...
btn[3].onclick = () => {
    ...
    for (let i = 0; i <= 100; i++) {
        const circleBox = Math.floor(Math.random()*2)
        const randomColor = Math.round(Math.random()*arrColors.length)
        const rndColor = arrColors[randomColor].name

        const element = document.createElement('div')
        element.style.width = "200px"
        element.style.height = "200px"
        element.style.backgroundColor = rndColor
        if(circleBox) element.style.borderRadius = "50%"
...
btn[4].onclick = () => {
    for (let i = 0; i < 100; i++) {
        ...
        let bombOrNot = Math.floor(Math.random()*2)
